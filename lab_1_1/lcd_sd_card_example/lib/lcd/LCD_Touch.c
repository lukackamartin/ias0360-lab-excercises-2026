...
{
    // Auto-release busy guard if save operation exceeds 5 seconds
    if (s_tp_save_busy && (to_ms_since_boot(get_absolute_time()) - s_tp_save_start_ms > 5000)) {
...
void TP_Save(void)
{
    ...
    // Non-blocking FIFO push with 500 ms timeout to prevent Core 0 freeze if Core 1 is unavailable
    if (!multicore_fifo_push_timeout_us(DATA_READY_FLAG, 500 * 1000)) {
        printf("TP_Save: Core 1 FIFO push timed out\r\n");
        GUI_DisString_EN(sLCD_DIS.LCD_Dis_Column - 120, 24,
...
void TP_DrawBoard(void)
{
    ...
    if (sTP_DEV.chStatus & TP_PRESS_DOWN)
    {
            ...
            sTP_Draw.Ypoint < 24)
        {
            ...
            while (!DEV_Digital_Read(TP_IRQ_PIN) && release_timeout++ < 300) {
                Driver_Delay_ms(10);
            }
            Driver_Delay_ms(50);
            sTP_Draw.Xpoint = 0;
